...
import { wordsCount, wordsDetect, wordsSplit } from '.'

const wordsIsExpected = (firstArr: string[], secondArr: string[]) =>
	firstArr.every((el: string, index: number) => secondArr[index] === el)
...
describe('Simple', function () {
	...
	it('English', function () {
		expect(wordsCount('Hello World')).toBe(2)
	})
	it('Chinese', function () {
		expect(wordsCount('你好，世界')).toBe(4)
	})
	it('English-Chinese', function () {
		expect(wordsCount('Hello, 你好。')).toBe(3)
	})
	it('English-Chinese-Japanese-Number', function () {
		expect(wordsCount("Let's say ‘你好’ 100 times per day, 勤勉")).toBe(10)
	})
})
...
describe('Basic', function () {
	it('English', function () {
		...
		expect(wordsCount(content)).toBe(17)
	})
	it('Chinese', function () {
		const content =
			'Google的免费服务可即时翻译英文和其他100多种语言的文字，短语和网页。'
		expect(wordsCount(content)).toBe(29)
	})
	it('Chinese-Traditional', function () {
		const content =
			'Google的免費服務可即時翻譯英文和其他100多種語言的文字，短語和網頁。'
		expect(wordsCount(content)).toBe(29)
	})
	it('Japanese', function () {
		const content =
			'Googleの無料サービスは、英語と100以上の他の言語との間で、単語、フレーズ、およびウェブページを即座に翻訳します。'
		expect(wordsCount(content)).toBe(48)
	})
	it('Korean', function () {
		const content =
			'Google의 무료 서비스는 단어와 구문, 웹 페이지를 영어와 100 개 이상의 다른 언어로 즉시 번역합니다.'
		expect(wordsCount(content)).toBe(38)
	})
	it('French', function () {
		const content =
		...
		expect(wordsCount(content)).toBe(23)
	})
	it('German', function () {
		const content =
	...
	it('Portuguese', function () {
		...
		expect(wordsCount(content)).toBe(21)
	})
	it('Russian', function () {
		const content =
	...
	it('Afrikaans', function () {
		...
		expect(wordsCount(content)).toBe(18)
	})
	it('Albanian', function () {
		...
		expect(wordsCount(content)).toBe(22)
	})
	it('Amharic', function () {
		const content =
	...
	it('Cebuano', function () {
		...
		expect(wordsCount(content)).toBe(33)
	})
	...
	it('Danish', function () {
		...
		expect(wordsCount(content)).toBe(16)
	})
	it('Dutch', function () {
		const content =
	...
	it('Hindi', function () {
		...
		expect(wordsCount(content)).toBe(24)
	})
	it('Hmong', function () {
		const content =
		...
		expect(wordsCount(content)).toBe(25)
	})
	...
	it('Maori', function () {
		...
		expect(wordsCount(content)).toBe(28)
	})
	it('Marathi', function () {
		const content =
	...
	it('Shona', function () {
		...
		expect(wordsCount(content)).toBe(14)
	})
	it('Sindhi', function () {
		const content =
	...
	it('Vietnamese', function () {
		...
		expect(wordsCount(content)).toBe(26)
	})
	it('Xhosa', function () {
		const content =
	...
	describe('Special', function () {
		it('Same punctuation multiple times: apostrophe', function () {
			...
			expect(wordsCount(content)).toBe(4)
		})
	...
	describe('Split and detector', function () {
		it('English', function () {
			...
			expect(wordsCount(content)).toBe(17)
			expect(wordsIsExpected(wordsSplit(content), splittedContent)).toBe(true)
			const { count: detectedCount, words: detectedWords } =
				wordsDetect(content)
			expect(detectedCount).toBe(17)
			expect(wordsIsExpected(detectedWords, splittedContent)).toBe(true)
		})
		it('Chinese', function () {
			const content =
				'Google的免费服务可即时翻译英文和其他100多种语言的文字，短语和网页。'
